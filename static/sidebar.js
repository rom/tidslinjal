    ...
    el.innerHTML = `
      ...
      <div class="sidebar-section">
        ...
        <div style="border-top:1px solid var(--border);padding-top:8px;margin-top:4px">
          <div style="font-size:var(--fs-xs);color:var(--text-dim);margin-bottom:6px">${t('security_session_hijack_desc')||'Session hijack protection — bind sessions to the client that created them so a stolen cookie cannot be replayed from a different device or network. DISABLED by default because it can kick legitimate users out behind reverse proxies or on networks that rotate IPs.'}</div>
          <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:var(--fs-sm);margin-bottom:8px;font-weight:600">
            <input type="checkbox" id="secHijackProtection" style="width:14px;height:14px;accent-color:var(--accent)">
            ${t('security_session_hijack_master')||'Enable session hijack protection'}
          </label>
          <div style="padding-left:22px;border-left:2px solid var(--border);margin-left:6px">
            <div style="font-size:10px;color:var(--text-dim);margin-bottom:4px">${t('security_session_hijack_sub_desc')||'Fine-tune which checks run when the master switch above is ON. These have no effect when the master is OFF.'}</div>
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:var(--fs-sm);margin-bottom:6px">
