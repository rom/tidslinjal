...
func (app *App) sendAutoReportEmail(s AutoReportSchedule) {
	...
	// Archive a copy in the report archive (local reports)
	app.archiveAutoReport(s, events, commentsMap)
}

func (app *App) archiveAutoReport(s AutoReportSchedule, events []Event, commentsMap map[int64][]EventComment) {
	now := time.Now()
	subject := fmt.Sprintf("Auto %s Report — %s", s.ReportType, now.Format("2006-01-02 15:04"))
	format := strings.ToLower(s.Format)
	if format == "" {
		format = "html"
	}
	...
	default:
		data = []byte(buildAutoReportHTML(s.ReportType, events, commentsMap))
		ext = "html"
		ct = "text/html"
	}

	fname := fmt.Sprintf("auto-%s-%s.%s", s.ReportType, now.Format("20060102-1504"), ext)
	app.saveReportToArchive(data, fname, ct, subject, "autogenerated", s.ReportType, s.CreatedBy)
}

// saveReportToArchive stores a generated report in the report archive.
func (app *App) saveReportToArchive(data []byte, filename, contentType, title, source, reportType string, userID int64) {
	dir := app.store.DataDir() + "/report_archive"
	_ = os.MkdirAll(dir, 0700)
	storedName := fmt.Sprintf("%d_%s", time.Now().UnixNano(), filename)
	if err := os.WriteFile(dir+"/"+storedName, data, 0600); err != nil {
		log.Printf("report-archive: failed to write %s: %v", filename, err)
		return
	}
	userName := ""
	if u, ok := app.store.GetUserByID(userID); ok {
	...
	if _, err := app.store.AddReportArchiveEntry(entry); err != nil {
		log.Printf("report-archive: failed to save entry %s: %v", filename, err)
	}
}
